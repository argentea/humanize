...
- hard blocking on gemini review failure or empty output
- open-question handling when `ask_gemini_question=true`

## Critical Rules

1. Never manually edit `state.md` or `finalize-state.md`.
2. Never skip a blocked gate result by declaring completion manually.
3. Never run ad-hoc gemini commands in place of the gate for phase transitions.
...
| `--gemini-model MODEL` | Gemini model | gemini-3.1-pro-preview |
| `--gemini-timeout SECONDS` | Gemini timeout | 5400 |
...
| `--claude-answer-gemini` | Let Claude answer open questions directly | false |
| `--agent-teams` | Enable agent teams mode | false |

Review phase uses `gemini --model gemini-3.1-pro-preview` with a git diff prompt.
