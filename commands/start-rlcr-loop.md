...
argument-hint: "[path/to/plan.md | --plan-file path/to/plan.md] [--max N] [--gemini-model MODEL] [--gemini-timeout SECONDS] [--track-plan-file] [--push-every-round] [--base-branch BRANCH] [--full-review-round N] [--skip-impl] [--claude-answer-gemini] [--agent-teams]"
allowed-tools:
...
- Otherwise, use the first positional argument (the first argument that does not start with `--` and is not a value following a known flag like `--max`, `--gemini-model`, `--gemini-timeout`, `--base-branch`, `--full-review-round`, `--plan-file`)
...
3. When you try to exit, Gemini reviews your summary
4. If Gemini finds issues, you receive feedback and continue
5. If Gemini outputs "COMPLETE", the loop enters **Review Phase**
6. In Review Phase, Gemini performs a git-diff-based code review
...
4. **Full Alignment Checks**: At configurable intervals (default every 5 rounds: rounds 4, 9, 14, etc.), Gemini conducts a comprehensive goal alignment audit. Use `--full-review-round N` to customize (min: 2)
...
5. **Trust the process**: Gemini's feedback helps improve the implementation

## Stopping the Loop

- Reach the maximum iteration count
- Gemini confirms completion with "COMPLETE", followed by successful code review (no `[P0-9]` issues)
...
1. **Implementation Phase**: Work on the plan, Gemini reviews your summary
2. **Review Phase**: After COMPLETE, Gemini checks code quality via git diff with `[P0-9]` severity markers
